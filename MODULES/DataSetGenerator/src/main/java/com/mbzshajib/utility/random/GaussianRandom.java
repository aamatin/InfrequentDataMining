...
//        }
        ...
        List<Integer> gaussianRandoms = gaussianRandom.getGaussianRandoms(10);
        for (Integer random : gaussianRandoms) {
            System.out.println("" + random);
        }
    }

    public List<Integer> getGaussianRandoms(int count) {
        List<Integer> result = new ArrayList<Integer>();
        for (int i = 0; i < count; i++) {
            double rand = Math.abs(minValue + random.nextGaussian() * variance);
            rand = new BigDecimal(rand).setScale(2, BigDecimal.ROUND_CEILING).doubleValue();
            if (rand > 1) {
                rand = rand - 1;
            }
            Integer randInteger = (int) rand;
            result.add(randInteger);
        }
        return result;
    }
}
